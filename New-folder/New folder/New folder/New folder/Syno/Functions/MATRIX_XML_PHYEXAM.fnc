--------------------------------------------------------
--  DDL for Function MATRIX_XML_PHYEXAM
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SNX_IWS2"."MATRIX_XML_PHYEXAM" 
    (   P_CASEID  number   ,
        P_LF varchar2  ) 
...
v_Str := v_Str || '         <title>Clock appears normal?</title>'  || LF;
v_Str := v_Str || '         <value>' ||  Get_InfoPathData('Mental_Status_Time',v_CaseId,'ST') || '</value>'  || LF;
v_Str := v_Str || '        </clockValid>'  || LF;
...
v_Str := v_Str || '                         <value>' ||  Get_InfoPathData('Physical_MSkel_Left_Aka, Physical_MSkel_Left_Bka, Physical_MSkel_Left_Great_Toe,  Physical_MSkel_Left_Other_Toes,  
