--------------------------------------------------------
--  DDL for Function MATRIX_XML_FUNCSTATUS_BK1
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SNX_IWS2"."MATRIX_XML_FUNCSTATUS_BK1" 
    (   P_CASEID  number   ,
        P_LF varchar2  ) 
...
v_Str := v_Str || '   <adlAssist>' ||  Get_InfoPathData('Health_Behaviors_STD_Tested',v_CaseId,'ST') || '</adlAssist>'  || LF;
...
v_Str := v_Str || '        <Status>' ||  Get_InfoPathData('ADL_Ambulation_Walker',v_CaseId,'ST') || '</Status>'  || LF;
