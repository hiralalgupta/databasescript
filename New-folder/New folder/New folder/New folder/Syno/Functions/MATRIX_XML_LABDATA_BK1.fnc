--------------------------------------------------------
--  DDL for Function MATRIX_XML_LABDATA_BK1
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SNX_IWS2"."MATRIX_XML_LABDATA_BK1" 
    (   P_CASEID  number   ,
        P_LF varchar2  ) 
...
v_Str := v_Str || '   <review>' ||  Get_InfoPathData('',v_CaseId,'ST') || '</review>'  || LF;
...
v_Str := v_Str || '   <labTests>' ||  Get_InfoPathData('',v_CaseId,'ST') || '</labTests>'  || LF;
...
v_Str := v_Str || '   <diagnosticTests>' ||  Get_InfoPathData('',v_CaseId,'ST') || '</diagnosticTests>'  || LF;
