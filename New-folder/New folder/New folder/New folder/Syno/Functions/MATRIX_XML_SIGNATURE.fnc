--------------------------------------------------------
--  DDL for Function MATRIX_XML_SIGNATURE
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SNX_IWS2"."MATRIX_XML_SIGNATURE" 
    (   P_CASEID  number   ,
        P_LF varchar2  ) 
...
v_Str := v_Str || '   </reviewers>'  || LF;
v_Str := v_Str || '   <reviewers>'  || LF;
v_Str := v_Str || '        <type>' || '' || '</type>'  || LF;
v_Str := v_Str || '        <friendlyType>' || '' || '</friendlyType>'  || LF;
v_Str := v_Str || '        <dateOfService>' || '' || '</dateOfService>'  || LF;
