--------------------------------------------------------
--  DDL for Function BUILD_IWS_URL_NEW
--------------------------------------------------------

  CREATE OR REPLACE FUNCTION "SNX_IWS2"."BUILD_IWS_URL_NEW" 
           ...
           WHEN v_ENV = 'AUTODETECT'
              then 
               ...
               EXCEPTION WHEN OTHERS THEN
                    BEGIN
                      v_URL_HEADER := 'NO SESSION FOUND for: >>>' || v('APP_SESSION') || '<<<';
                    END;
               END;
            ...
            --  v_match_str := lower(substr(v_match_str,1,v_match_split_pos-1));
               ...
               BEGIN
                  ...
                  EXCEPTION WHEN OTHERS THEN
                       v_URL_HEADER := 
                          'no APEX_HOST_MATCH entry for: >>>' || v_Org_ENV || '<<<';
               END;
