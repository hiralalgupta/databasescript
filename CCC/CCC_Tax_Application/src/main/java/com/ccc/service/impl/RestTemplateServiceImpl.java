...
/*	@Autowired
	...
	public <T> T postDataToWS(String url, HttpMethod httpMethod, Object requestEntity, Class<T> responseEntity) {
		...
		try {
			...
			ErrorResponseWrapper errorResponse=(ErrorResponseWrapper) e.getResponseEntity().getBody();
			showErrorInfoFromRestServer(errorResponse.getErrorResponse());
			DataResponse errResponse=new DataResponse();
			errResponse.setErrorResponse(errorResponse.getErrorResponse());
