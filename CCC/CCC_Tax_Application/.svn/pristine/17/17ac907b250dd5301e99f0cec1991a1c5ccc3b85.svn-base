...
public class SecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().requireCsrfProtectionMatcher(csrfRequestMatcher).and().authorizeRequests().antMatchers("/wel**").permitAll()
		.antMatchers("/tax**").access("hasRole('TAXVIEW')")
		.antMatchers("/modify**").access("hasRole('ADMIN')")
		.antMatchers("/lookUpTaxCalculate").access("hasRole('ZIPLOOKUP')")
