...
public class SaleTaxServiceImpl implements SaleTaxService {
	...
	public List<TaxObject> getTaxes(FormData jsonData) throws UniqueCountyNotFoundException {
		...
		Integer carSeats = jsonData.getVehicleSeats(); 
		String vehicleType = jsonData.getVehicleType(); 
		String registrationDate = jsonData.getRegistrationDate();
		String totaledDate = jsonData.getLossDate();		
		String cName = jsonData.getCounty();
		
		List<TaxObject> result= new ArrayList<TaxObject>();
		if(cName == null) {
			logger.debug("calling saleTaxDao.getCountyData with zip: "+zip+ " and plus as "+plus4+" and request id "+jsonData.getTransactionId());
			List<County>  countyData = saleTaxDao.getCountyData(Integer.valueOf(zip), Integer.valueOf(plus4),rName);
			logger.debug("Successfully called saleTaxDao.getCountyData with zip: "+zip+ " and plus as "+plus4+" and request id "+jsonData.getTransactionId());
			if(countyData != null && countyData.size()>1) {
				logger.debug("More than one county for zip: "+zip+ " and plus as "+plus4+" and request id "+jsonData.getTransactionId());
		...
		logger.debug("calling saleTaxDao.getSaleTax with zip: "+zip+ " and plus as "+plus4+" and request id "+jsonData.getTransactionId());
		if(rName.equals("WA")||rName.equals("TN")) {
			list=saleTaxDao.getWASaleTax(Integer.valueOf(plus4),Integer.valueOf(zip),rName,cName.toString(), modelMake, modelName, Integer.valueOf(modelYear),modelTrim,plateType,totaledDate);
		}else if(rName.equals("TX")) {
			list=saleTaxDao.getTXSaleTax(Integer.valueOf(plus4),Integer.valueOf(zip),rName,cName.toString(), modelMake, modelName, Integer.valueOf(modelYear),modelTrim,plateType,totaledDate);
		}
		else
			list=saleTaxDao.getSaleTax(Integer.valueOf(plus4),Integer.valueOf(zip),rName,cName.toString(), modelMake, modelName, Integer.valueOf(modelYear),modelTrim,plateType,totaledDate);
		
		logger.debug("calling saleTaxDao.getCountyData with zip: "+zip+ " and plus as "+plus4+" and request id "+jsonData.getTransactionId());
		//int weight = StringUtils.isEmpty(carWeight)?saleTaxDao.getVehicleWeight(modelMake, modelName, modelYear, modelTrim):carWeight;
		
		VehicleInfo vehicleOb = saleTaxDao.getVehicleWeightSeat(modelMake,modelName,modelYear, modelTrim);
		int weight = StringUtils.isEmpty(carWeight)?vehicleOb.getWeight():carWeight;
		int seats = StringUtils.isEmpty(carSeats)?vehicleOb.getSeats():carSeats;
		
		logger.info("weight:: "+weight+"and seats:: "+seats+" for request id "+jsonData.getTransactionId());
		logger.debug("calling saleTaxDao.getVehicleTaxandFees with vehicleType: "+vehicleType+ " and weight as "+weight+" and request id "+jsonData.getTransactionId());
		...
		if(rName.equals("WA"))
			...
			regFeesByVehicleType = saleTaxDao.getRegistrationFeesForTX(vehicleType, weight,registrationDate,modelYear,rName,cName);
		else if(rName.equals("GA"))
			regFees = saleTaxDao.getRegistrationFeesForGA(vehicleType, weight,rName);
		else if(rName.equals("TN"))
			regFees = saleTaxDao.getRegistrationFeesForTN(vehicleType, weight,rName,seats);
		else
			regFees = saleTaxDao.getGenericFeesData(rName, cName, modelMake, modelName, modelYear, modelTrim);
		
		
		logger.debug("Successfully called  saleTaxDao.getVehicleTaxandFees with vehicleType: "+vehicleType+ " and weight as "+weight+" and request id "+jsonData.getTransactionId());
		...
		totalTitleFees = totalTitleFees + result.parallelStream().filter(tax -> tax.getType().equals("TF") && tax.getUnit().equals("$"))
