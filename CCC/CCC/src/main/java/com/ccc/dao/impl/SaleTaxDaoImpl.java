...
public class SaleTaxDaoImpl extends AbstractDao<Long, User>  implements SaleTaxDao  {
	...
	public List<Object[]> getSaleTax(Integer plus4,Integer zip,String rName,String cName,String modelMake,String modelName,Integer modelYear, String modelTrim,String plateType,String totaledDate) {
		...
		StringBuilder sqlAppender=new StringBuilder("SELECT LABEL ,LC.VALUE,U.VALUE AS UOM,URL,TYPE FROM "
				...
				+ " LEFT OUTER JOIN (SELECT URL,TAXID,START_PERIOD,END_PERIOD FROM TAXAFFECTEDURL TEU,TAXURL TU WHERE TEU.TAXURLID=TU.TAXURLID  AND (STR_TO_DATE(:totaledDate, '%m/%d/%Y') "
	...
	public List<Object[]> getWASaleTax(Integer plus4,Integer zip,String rName,String cName,String modelMake,String modelName,Integer modelYear, String modelTrim,String plateType,String totaledDate) {
		...
		*/
		StringBuilder sqlAppender=new StringBuilder("SELECT LABEL ,LC.VALUE,U.VALUE AS UOM,URL,TYPE FROM " + 
				...
				" INNER JOIN COUNTYCITYTAX CCT ON CCT.TAXID=T.TAXID  AND CCT.ISACTIVE='Y' " + 
				" INNER JOIN COUNTY C ON C.COUNTYID=CCT.COUNTYID " + 
				" INNER JOIN (SELECT ZIP,COUNTYID,REGIONID,AREACODE FROM LOCATION WHERE (:plus4 BETWEEN ZIP4LOW AND ZIP4HIGH)  AND ZIP=:zip) L ON L.COUNTYID=CCT.COUNTYID "
	...
	public List<Object[]> getRegistrationFeesForGA(String vehicleType, int weight,String state) {
		...
		StringBuilder sqlAppender=null;
		
			sqlAppender=new StringBuilder("SELECT F.FEESNAME AS LABEL ,CASE WHEN V.ISWEIGHT=1 THEN W.FEES ELSE V.INITIALFEES END AS VALUE,U.VALUE AS UOM,F.FEESURL AS URL,FC.CODE AS TYPE "+
	...
	@Override
	public VehicleInfo getVehicleWeightSeat(String modelMake, String modelName,Integer modelYear, String modelTrim) {
		// TODO Auto-generated method stub
		String trimData = !StringUtils.isEmpty(modelTrim)? " AND UPPER(M.TRIM)=UPPER(:modelTrim) ":" ";
		Session session =getSession();
		StringBuilder sqlAppender=new StringBuilder("SELECT DISTINCT M.WEIGHT_POUND,M.SEATS FROM MODELS M,MODELMAKE MM,VEHICLEMODELS VM"
				+ " WHERE  MM.MODELMAKEID=M.MODELMAKEID AND VM.VEHICLEMODELID=M.VEHICLEMODELID AND UPPER(MM.MODELMAKE)=UPPER(:modelMake) "
				+ " AND M.YEAR=:modelYear AND UPPER(VM.MODELNAME)=UPPER(:modelName)").append(trimData);
		...
		//Double res= (Double)query.getSingleResult();
		List<Object[]> list =query.getResultList();
		double weightInDouble = Double.valueOf(list.get(0)[0].toString());
		int seats = Integer.valueOf(list.get(0)[1].toString());
		VehicleInfo vehicleInfo = new VehicleInfo();
		vehicleInfo.setSeats(seats);
		vehicleInfo.setWeight((int) Math.round(weightInDouble));
		
		return vehicleInfo;
	}

	@Override
	public List<Object[]> getRegistrationFeesForTN(String vehicleType, int weight,String state,int seats) {
		Session session =getSession();
				
		StringBuilder sqlAppender=null;
		
			 sqlAppender=new StringBuilder("SELECT F.FEESNAME AS LABEL , CASE WHEN F.ISVEHICLETYPE=1 THEN V.INITIALFEES ELSE F.FEESVALUE END AS VALUE,U.VALUE AS UOM,CASE WHEN F.ISVEHICLETYPE=1 THEN V.URL ELSE F.FEESURL END AS URL,FC.CODE AS TYPE "); 
			 sqlAppender.append("FROM UOM U,FEESCATEGORY FC,REGION R, FEES F  ");
			 sqlAppender.append("LEFT OUTER JOIN (SELECT V.NAME,COALESCE(V.INITIALFEES,VA.FEES) AS INITIALFEES,VF.FEESID,V.URL,V.MIN_SEAT,V.MAX_SEAT FROM VEHICLETYPEFEES VF,VEHICLETYPE V LEFT OUTER JOIN VEHICLEAGE VA ON VA.VEHICLETYPEID=V.VEHICLETYPEID AND :seats BETWEEN MINSEAT AND MAXSEAT ");
			 sqlAppender.append("WHERE V.VEHICLETYPEID=VF.VEHICLETYPEID AND V.NAME=:vehicleType");
			 sqlAppender.append( ") V ON V.FEESID=F.FEESID ");
			 sqlAppender.append( "WHERE R.REGIONID=F.REGIONID AND R.NAME=:state AND F.FEESCATEGORYID=FC.FEESCATEGORYID AND ");
			 sqlAppender.append("F.FEESCATEGORYID=2 AND U.UOMID=F.UOMID");
